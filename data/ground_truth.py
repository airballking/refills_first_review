...
def get_pose_stamped(node, now):
    if not node.parent:
        raise RuntimeError("Root of the tree does not have meaningful PoseStamped.")
    msg = PoseStamped()
    msg.header.stamp = now
    msg.header.frame_id = node.parent.name
#    msg.child_frame_id = node.name
    msg.pose.orientation.w = 1.0
    msg.pose.position = to_msg(node.pos)
    return msg
...
def assert_into_knowrob(data):
    if data.parent:
        raise RuntimeError("'Data' is not anytree root.")

    # add shelf system
    my_knowrob = KnowRob()
    now = rospy.Time.now()
    shelf_system_id = my_knowrob.add_shelf_system()

    # add shelf meters
    shelf_meters = findall(data, lambda node: node.type is shelf_meter_key)
    shelf_meter_dict = OrderedDict()
...
#
#       # separators = findall(shelf_meter, lambda  node: node.type is separator_key)
#        print('{} has {}'.format(shelf_meter.name, shelf_layers))

#    for node in at.LevelOrderIter(data):
#        print node
...
def system_node(name, parent, x, y, z):
    return at.Node(name, parent=parent, pos=kdl.Vector(x, y, z), type=shelf_meter_key)
...
def populate_ground_truth():
    root = at.Node("map", type=shelf_system_key)
